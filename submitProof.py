...
def prove_merkle(merkle_tree, random_indx):
    ...
    # TODO YOUR CODE HERE
    index = random_indx
    for level in merkle_tree[:-1]:  # Iterate all levels except the root
        # If odd number of nodes at this level, the last node is paired with itself
        if len(level) % 2 == 1:
            level = level + [level[-1]]
...
def send_signed_msg(proof, random_leaf):
    ...
    # Convert proof items to bytes32 (they are already bytes, ensure they are bytes32)
    proof_bytes32 = [bytes(p) if not isinstance(p, bytes) else p for p in proof]
    leaf_bytes32 = bytes(random_leaf) if not isinstance(random_leaf, bytes) else random_leaf
